...
export default function RiskCategoriesTab({ data, onReload }: Props) {
  ...
  const remove = async (c: RiskCategory) => {
    if (!confirm(t.riskCatDeleteConfirm(c.name))) return
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold" style={{ color: 'var(--color-text-body)' }}>{t.riskCategories}</h2>
        <button onClick={openCreate} className="btn-primary"><Plus size={15} /> {t.riskCatAdd}</button>
      </div>

      {data.riskCategories.length === 0 ? (
        <div className="card p-10 text-center" style={{ color: 'var(--color-text-body)', opacity: 0.5 }}>{t.riskCatNone}</div>
      ) : (
          ...
          {data.riskCategories.map(c => (
            <div key={c.id} className="flex items-center gap-3 px-4 py-3 hover:bg-black/5 group">
              ...
              <span className="text-xs opacity-50" style={{ color: 'var(--color-text-body)' }}>
                {t.riskCatCount(data.risks.filter(r => r.category_id === c.id).length)}
              </span>
      ...
      {modal && (
        <Modal title={modal.mode === 'create' ? t.riskCatNew : t.riskCatEdit} onClose={() => setModal(null)} size="sm">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text-body)' }}>{t.riskCatName}</label>
              <input className="input" value={name} onChange={e => setName(e.target.value)} autoFocus
                onKeyDown={e => e.key === 'Enter' && save()} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--color-text-body)' }}>{t.riskCatColor}</label>
